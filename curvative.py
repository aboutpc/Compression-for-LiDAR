...
def density_cal(p):
    [p1,p2,p3,p4] = p
    r1 = (p1[0]-p2[0])**2+(p1[1]-p2[1])**2+(p1[1]-p2[1])**2
    r3 = (p3[0]-p2[0])**2+(p3[1]-p2[1])**2+(p3[1]-p2[1])**2
    r4 = (p4[0]-p2[0])**2+(p4[1]-p2[1])**2+(p4[1]-p2[1])**2
    if r1>r3:
        if r1>r4:
            return 4/(r1*3.1415926)
        else:
            return 4/(r4*3.1415926)
    else:
        if r3>r4:
            return 4/(r3*3.1415926)
        else:
            return 4/(r4*3.1415926)
...
def curvature_density_del_to_anynum(myinput,thre=1e-6,point_num=48128):
    curvature = np.array([])
    density = np.array([])
    new_myinput = np.array([[0,0,0]]) 
    ...
    for i in range(1,myinput.shape[0]-2):
        ...
        p1=[myinput[i-1],myinput[i],myinput[i+1],myinput[i+2]]
        this_curvature=curvature_cal(p1)
        this_density=density_cal(p1)
        if i is 1:
            new_myinput=[myinput[i]]
            curvature=np.insert(curvature,0,this_curvature,0)
            density=np.insert(density,0,this_density,0) 
            
        if count==0:
            curvature = np.append(curvature,this_curvature)
            density = np.append(density,this_density)
            new_myinput = np.append(new_myinput,[myinput[i]],0)
            if i is myinput.shape[0]-3:
                curvature=np.insert(curvature,-1,this_curvature,0)
                density=np.insert(density,-1,this_density,0)
                new_myinput=np.insert(new_myinput,0,myinput[i],0)
    ...
    return new_myinput,curvature,density
